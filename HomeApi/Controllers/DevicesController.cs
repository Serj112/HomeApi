...
namespace HomeApi.Controllers
{
    ...
    public class DevicesController : ControllerBase
    {
        ...
        //Удаление по серийному номеру
        [HttpDelete]
        [Route("DeleteBySerial{serialID}")]
        public async Task<IActionResult> DeleteBySerialId([FromRoute] Guid serialId)
        {
            var device = await _devices.GetDeviceById(serialId);
            if (device == null)
                return StatusCode(400, $"Ошибка: Устройство с серийным номером {serialId} не существует!");

            await _devices.DeleteDevice(device);

            return StatusCode(200, $"Устройство удалено! Имя устройства - {device.Name}, Серийный номер - {device.SerialNumber}, Комната подключения - {device.Room.Name}");
        }
